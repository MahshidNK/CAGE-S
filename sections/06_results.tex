...
the predicted probabilities, rather than only changing their ranking.
...
\subsection{End-to-end comparison on the natural workload}
...
\begin{table*}[t]
\centering
\caption{End-to-end results on the natural test workload
(11,629 events). Rates are percentages. Next-state unsafe denotes the
recorded state of the selected node at $t+\Delta$.}
...
The comparison with the current-state learned scheduler isolates the
benefit of temporal features. Policy-current-HGB-Q95 migrates 42.850\%
of events and leaves a temporal-violation rate of 0.370\%. CAGE-S reduces
...
temporal screening. CAGE-S increases migration by only 0.920 percentage
...
Policy-temporal-no-projection already eliminates policy and temporal
...
\begin{table*}[t]
\centering
\caption{End-to-end results on the service-balanced test workload
(3,787 events). Rates are percentages.}
...
The balanced workload reproduces the natural-workload pattern. CAGE-S
satisfies all policy, temporal-risk, and Q95 conditions with a migration
rate of 40.190\% and a next-state unsafe rate of 0.106\%.
Policy-current-HGB-Q95 migrates more events, at 43.200\%, while leaving
...
Least-loaded-policy-Q95 concentrates 39.959\% of migrations on its ten
...
index to 0.115. CAGE-S retains a similarly distributed assignment pattern
while adding temporal screening and pre-commit verification.

\begin{figure*}[t]
  \centering
  \includegraphics[width=0.72\textwidth]
  {migration_risk_tradeoff.pdf}
  \caption{Migration rate and next-state unsafe rate for representative
  scheduling methods on the natural and service-balanced workloads.
  Lower values are preferred on both axes.}
  \label{fig:migration-risk-tradeoff}
\end{figure*}

\subsection{Contribution of individual scheduler components}

The complete comparison reveals four distinct component effects. Policy gating is indispensable for sensitive placement. Removing it from
the temporal scheduler produces policy-violation rates of 25.411\% and
24.399\% on the natural and balanced workloads, respectively. Temporal history improves both decision efficiency and operational
screening. Relative to Policy-current-HGB-Q95, CAGE-S reduces migration
by 3.732 percentage points on the natural workload and by 3.010
percentage points on the balanced workload, while eliminating the
remaining temporal-threshold violations. Fairness-aware selection prevents destination concentration. On the
natural workload, replacing least-loaded selection with fair
policy-Q95 selection increases migrated-destination Jain fairness from
0.009 to 0.291 and reduces the top-ten destination share from 13.282\%
to 0.450\%. On the balanced workload, the corresponding Jain values are
0.002 and 0.115. The pre-commit projection provides the final executability check. Compared
with Policy-temporal-no-projection, it removes Q95-violation rates of
1.144\% and 0.792\% on the natural and balanced workloads for migration
increases of 0.929 and 0.792 percentage points, respectively.
...
natural-workload slack falls to two candidates. CAGE-S therefore uses
...
CAGE-S minus the corresponding baseline. For migration and violation
rates, an interval below zero favours CAGE-S.
...
Relative to the current-state learned scheduler, CAGE-S achieves
...
of Q95 violations. The confidence interval for the natural-workload next-state difference
against Current-HGB reaches zero at its upper boundary. The direction of
the effect remains favourable, whereas the evidence is strongest for the
migration and temporal-violation differences.
...
\begin{figure}[!b]
  \centering
  \includegraphics[width=\columnwidth]{seed_sensitivity.pdf}
  \caption{Mean and 5th--95th percentile migration rates across
  30 deterministic policy overlays.}
...
\begin{center}
  \includegraphics[width=\columnwidth]{runtime_latency.pdf}
  \captionof{figure}{Distribution of complete online-core latency on the natural
  and service-balanced test workloads.}
